...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // We loop through each filter button and add a click listener
...
// When a filter button is clicked, we update the active filter and re-render the list
function handleFilterClick(event) {
    // Get the filter value from the clicked button's data-filter attribute
    const selectedFilter = event.target.dataset.filter;
    currentFilter = selectedFilter;

    // Update the active class on filter buttons so the user can see which filter is selected
    filterButtons.forEach(function(button) {
...
function handleClearCompleted() {
    // Keep only the todos that are NOT completed
...
function renderTodos() {
    ...
    if (filteredTodos.length === 0) {
        todoList.innerHTML = '<li class="empty-state">No matching todos for this filter.</li>';
        return;
    }

    // Create and append only the filtered todo items
...
// Returns the todos that match the current filter
...
function updateTodoCount() {
    ...
    // Get the total number of all todos
    const totalTodos = todos.length;

    // Build a message showing progress
    const countMessage = `${activeTodos} of ${totalTodos} items remaining`;

    // Update the displayed count message
    todoCount.textContent = countMessage;
}
